...
async def update_user(db:AsyncSession,user_data:UserUpdateRequest,username:str):
    #user_data是pydantic类型，需要得到字典再进行**解包
    # user_data.model_dump()进行转换成字典
    #exclude_unset=True,exclude_none=True,用户没有设置修改的字段不更新，使用系统默认值
    ...
    result1=await get_user_by_username(db,username)
    return result1


async def change_password(db:AsyncSession,user:User,old_password:str,new_password:str):
    security.verify_password(old_password,user.password)
    if not security.verify_password(old_password,user.password):
        raise HTTPException(status_code=400,detail="旧密码错误")
    hashed_new_pwd=security.get_hash_password(new_password)
    user.password=hashed_new_pwd
    db.add(user) #确保可以提交，规避session过期或关闭不能提交的问题
    await db.commit()
    await db.refresh(user)
    return True
